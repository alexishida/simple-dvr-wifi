...
function MonitoringTile({
  ...
  const toggleRecording = (): void => {
    if (recording) {
      void window.api.recordings.stop(camera.id).then((result) => {
        ...
        else setSnapshotStatus("Falha ao finalizar a gravação.");
      });
    } else {
  ...
  return (
    <article
      className={`monitor-tile${selectedPtzCameraId === camera.id ? " monitor-tile-selected" : ""}`}
      role="listitem"
      aria-label={`Vídeo da câmera ${camera.name}`}
      onClick={() => onPtzSelect(camera)}
    >
