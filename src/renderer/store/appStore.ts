  ...
  switch (mutation.kind) {
    case "deactivate":
      return cameras.map((c) =>
        c.id === mutation.id
          ? { ...c, active: false, status: "disabled" as const }
          : c,
      );
    case "reactivate":
      return cameras.map((c) =>
        c.id === mutation.id
          ? { ...c, active: true, status: "disconnected" as const }
          : c,
      );
    case "updateCredentials":
      return cameras.map((c) =>
        c.id === mutation.id ? { ...c, hasCredential: true } : c,
      );
    case "remove":
...
function restoreSnapshot(
  ...
  if (!existing) return [...cameras, snapshot];
...
export const useAppStore = create<AppState>((set) => ({
  ...
  setCameras: (cameras) =>
    set((state) => ({
      cameras,
      hydrated: true,
      fullscreenCamera: state.fullscreenCamera
        ? (cameras.find((camera) => camera.id === state.fullscreenCamera?.id) ??
          null)
        : null,
    })),
  ...
  closeFullscreen: () =>
    set({ fullscreenCamera: null, fullscreenProfile: "main" }),
...
export async function runCameraMutation(
  mutation: CameraMutation,
): Promise<boolean> {
  ...
  try {
    switch (mutation.kind) {
      ...
      case "reactivate":
        ...
        break;
      case "updateCredentials":
        ok = (await window.api.cameras.updateCredentials({ id: mutation.id }))
          .ok;
        break;
      case "remove":
