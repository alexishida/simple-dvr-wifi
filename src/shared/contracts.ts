...
export const CameraEditDetailsSchema = z
  .object({
    id: CameraIdSchema,
    name: CameraNameSchema,
    host: HostSchema,
    port: PortSchema.nullable(),
    onvifUrl: z.string().max(2048).nullable(),
    rtspUrl: z.string().max(2048).nullable(),
    rtspSubUrl: z.string().max(2048).nullable(),
    snapshotUrl: z.string().max(2048).nullable(),
