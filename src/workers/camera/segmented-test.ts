...
export type SegmentName =
  'reachability' | 'authentication' | 'onvif' | 'media' | 'rtsp' | 'snapshot' | 'ptz' | 'codec'
...
export async function runSegmentedTest(
  ...
  const probeHttp =
    deps.probeHttp ?? (async (url) => (await import('./probes.js')).probeHttp({ url }))
  const probeRtsp =
    deps.probeRtsp ?? (async (url) => (await import('./probes.js')).probeRtsp({ url }))
  ...
  if (deps.onvifAdapter) {
    try {
      ...
      if (onvifInfo.capabilities.onvif === 'error') {
        ...
        segments.set(
          'onvif',
          ok('onvif', `Identidade e ${onvifInfo.profiles.length} perfil(is) detectados.`),
        )
      } else {
  ...
  if (onvifInfo) {
    ...
    const codecs = new Set(onvifInfo.profiles.map((p) => p.codec).filter(Boolean) as string[])
    if (codecs.size > 0) {
    ...
    segments.set(
      'ptz',
      onvifInfo.ptzSupported ? ok('ptz', 'PTZ configurado.') : warn('ptz', 'PTZ não confirmado.'),
    )
  } else {
  ...
  return { segments: ordered.map((segment) => segments.get(segment)!), summary }
}
