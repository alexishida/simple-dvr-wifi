...
export class SqliteWorker {
  ...
  async dispatch(request: DbRequest): Promise<DbResponse> {
    const reply = (value: unknown): DbResponse => ({ id: request.id, ok: true, value })
    const error = (
    ...
    switch (request.op) {
      ...
      case 'open': {
        const payload = request.payload as { dbPath?: string; backupDir?: string } | undefined
        if (!payload?.dbPath) return error('VALIDATION_ERROR', 'dbPath obrigatório.')
      ...
      case 'backup.export': {
        ...
        if (!payload?.destination) return error('VALIDATION_ERROR', 'destination obrigatório.')
    ...
    try {
      switch (request.op) {
        case 'camera.create': {
          const p = request.payload as Parameters<CameraRepository['create']>[0]
          return reply(r.cameras.create(p))
        }
        case 'camera.get': {
          const p = request.payload as { id: string }
          const camera = r.cameras.getById(p.id)
          return camera ? reply(camera) : error('NOT_FOUND', 'Câmera não encontrada.')
        }
        case 'camera.list':
          return reply(r.cameras.list())
        case 'camera.update': {
          const p = request.payload as { id: string } & Parameters<CameraRepository['update']>[1]
          const camera = r.cameras.update(p.id, p)
          return camera ? reply(camera) : error('NOT_FOUND', 'Câmera não encontrada.')
        }
        case 'camera.deactivate': {
          const p = request.payload as { id: string }
          return r.cameras.deactivate(p.id)
            ? reply({ deactivated: true })
        ...
        case 'camera.setCapabilities': {
          const p = request.payload as { cameraId: string } & Record<string, unknown>
          r.capabilities.upsert(p.cameraId, p as Parameters<CapabilityRepository['upsert']>[1])
          return reply({ stored: true })
        }
        ...
        case 'credential.set': {
          const p = request.payload as { cameraId: string; service: string } & Record<
            string,
            unknown
          >
        ...
        case 'credential.get': {
          ...
          return credential ? reply(credential) : error('NOT_FOUND', 'Credencial não encontrada.')
        }
        case 'credential.has': {
          const p = request.payload as { cameraId: string }
        ...
        case 'diagnostic.append': {
          const p = request.payload as Parameters<DiagnosticRepository['append']>[0]
