...
const FULL_INFO: CameraOnvifInfo = {
  ...
  identity: { manufacturer: 'Acme', model: 'Cam', firmwareVersion: '1', serialNumber: 'SN' },
  capabilities: {
    onvif: 'supported',
    rtsp: 'supported',
    snapshot: 'supported',
...
function deps(overrides: Partial<SegmentedTestDependencies> = {}): SegmentedTestDependencies {
  return {
...
describe('segmented connection test', () => {
  it('reports independent results for a fully working camera', async () => {
    ...
    const statuses = Object.fromEntries(result.segments.map((s) => [s.segment, s.status]))
  ...
  it('reports auth failure without hiding other results', async () => {
    const result = await runSegmentedTest(
      {
        onvifUrl: 'http://cam.local/onvif',
        rtspUrl: 'rtsp://cam.local/main',
  ...
  it('respects limited concurrency via mapLimit', async () => {
    const order: number[] = []
    const { mapLimitExports } = await import('../src/workers/camera/segmented-test.js')
